...
public class PokemonOpponent : MonoBehaviour
{
    ...
    // Référence à l'objet Pokémon
    [SerializeField] private PokemonObj PokemonObj;

    // Méthode pour initialiser les points de vie actuels du Pokémon
    public void InitCurrentLife(PokemonData pokemonData)
    {
        pokemonData.GetCurrentHp = pokemonData.GetHP;
    }

    // Méthode pour initialiser les points de statistiques du Pokémon
    ...
    public void TakeDamage(PokemonData pokemonData, int damage)
    {
        ...
        {
            if (pokemonData.GetWeaknesses[i] == PokemonObj.pokemonData.GetTypePokemon)
            {
        ...
        {
            if (pokemonData.GetResistances[i] == PokemonObj.pokemonData.GetTypePokemon)
            {
                pokemonData.GetCurrentHp -= damage / 2;
            }
        }
    ...
    // Méthode pour faire attaquer le Pokémon l'adversaire
    public void AttackOpponent(PokemonData pokemonData)
    {
        if (IsPokemonAlive(pokemonData))
        {
            PokemonObj.TakeDamage(PokemonObj.pokemonData, GetAttackDamage(pokemonData));
        }
        else
        {
    ...
    // Méthode Start pour commencer à attaquer l'adversaire aléatoirement
    private void Start()
    {
        StartCoroutine(AttackOpponentRandomly());
    }

    // Coroutine pour attaquer l'adversaire aléatoirement
